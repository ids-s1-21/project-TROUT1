...
<font size = '6'>As you can see, 'No. of Bedrooms', 'No. of Bathrooms' and 'No. of Receptions' are the same as each other in this dataset. By looking up some of the listings we determined that this number represents the 'No. of Bedrooms'</font> 
...
## Prices grouped by region and type of housing (to look at if the difference in mean price between the outer and further outer regions are concentrated into one type of housing, its not)


```{r echo = FALSE, warning = FALSE, message=FALSE}
London %>%
  filter(`House Type` %in% c("Flat / Apartment", "House", "New development" )) %>%
  filter(large_region %in% c("Further Outer", "Outer")) %>%
...
- Generally as you move further from the center of london house prices decrease and the price per square foot decreases. However our data tells us that on average homes in the further outer sector are more expensive then houses in the outer sector , this did not fit with the trend and is because both houses and apartments are more expensive on average in the further outer region,  as shown in the previous slide this could be because of the prevalence of cheaper terrace hosing in the outer region.

- There is a fairly simplistic linear relationship between area of a home and the price of a home this can be shown as the linear regression model between the two has a adjusted r squared value of ...... . When the points are plotted on a scatter plot you can see a clear positive correlation . 



## Limitations with our data

- The data set was scraped poorly. It had repeated values under the number of bathrooms,bedrooms and reception variables. After removing the number of bathroom and foyer variables our data set contained a limited amount of data to manipulate.
