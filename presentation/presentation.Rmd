...
# library(png)
# pp <- readPNG("presentation_files/Support-files/London-Price-Map.png")
# plot.new() 
...
```
Due to the Graph being cluttered due to over plotting i instead used glance to show there is a strong correlation between area and price of housing. We can tell this this from the high adjusted r squared value of 0.575(to 3dp).

## Correlation between Area and Price over the Middle region

```{r data for housetype, echo = FALSE, warning = FALSE, message=FALSE}
London %>%
  filter(large_region == "Middle") %>%
ggplot(aes(x = `Area in sq ft`, y = Price)) +
  geom_point() +
  geom_smooth(method = "lm", color = "blue", se = FALSE) +
...
```
The increase in adjusted r squared from 0.575 to 0.730 shows that limiting the data to 1 large region makes the strength of the correlation between area and price much higher.
...
- There is a fairly simplistic linear relationship between area of a home and the price of a home this can be shown as their linear regression model between the two has a high adjusted r squared value of 0.575 . When the points are plotted on a scatter plot you can also see a clear positive correlation . 
