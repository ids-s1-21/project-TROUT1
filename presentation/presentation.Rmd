---
title: "Explore How Multiple Factors Affect the Housing Price in London"
...
## Our goal is to find out what and how are region, house type, and total aera affects housing price in London {data-background-image="img/confetti.jpg"}

# About the Data

- We have got over 3400 housing price data for houses in all regions in greater London. However, since the majority of the houses are located in London, we will only consider houses in London, but not greater London.^1^

- We cleaned the data by:
1. Filter out all houses that are not in London.
2. Remove column "No. of bathrooms" and "No. of receptions."^2^
3. Extract the Outward Code in the "Postal Code" variable.^3^
4. Add variable: "price_per_sq_ft" = "Price"/"Area in sq ft"
5. Divide them into 4 large regions.^4^
...
2. Because of the flaw of the original data, the variable "No. of bedrooms", "No. of bathrooms", and "No. of receptions are all the same for each observation. We checked some famous houses and figured out that this may represents the number of bedrooms
3. Outward Code is always the first two to four characters in postal code. i.e: SW9 9BT has outward code of SW9. This always indicates which region does the house locate.
...
- In the beginning of the analysis, we will start identify how the location of the house affects the price.
I will divide the region in London into 4 big regions: from city center to further places, see the image below.

- Different marked colour represents different regions:
1. Red represents group "Central"
2. Blue represents group "Middle"
3. Red represents group "Outer"
4. Unmarked region represents group "Further outer"
